...
class User {
    ...
    private $dbName     = 'oauth_users'; //MySQL_Database_Name
...
public function updatePosts($userData,$count){
	...
	// ---------------------------------
	// update galleryname in a user record
public function updateGalleryname($userData,$galleryname){
	if(!empty($userData)){
		// Check whether user data already exists in database
		//echo "oauth_id : ".$userData['oauth_id']."<br>";
		$prevQuery = "SELECT * FROM `".$this->userTbl."` WHERE oauth_id = '".$userData."'";
		...
		if($prevResult->num_rows == 1){
			// Update user data if already exists
			$query = "UPDATE `".$this->userTbl."` SET gallery_name = '".$galleryname."', modified = NOW() WHERE oauth_id = '".$userData."'";
			$update = $this->db->query($query);
		}
		// Get user data from the database
		$result = $this->db->query($prevQuery);
